-- | An copy of interpreter for Procedural Imperative Programming Language (PIPL):
...
-- - Out/Upd parameters are by copy-in/copy-out: a local variable with the parameter's name is a copy of the argument variable, and the copy is used within the procedure.
...
-- - Obs parameters are by value (copy-in) to a local variable with the parameter's name.
-- - Out/Upd parameters are by value (copy-in) to a local variable with the parameter's name,
--   and then by reference (copy-out) back to the original argument variable.
...
buildArgs oldenv (param@(vpname,(Obs,tname)):params) (arg@(Right e):args) state =  state'
  ...
  val = eval oldstate e
  state' = buildArgs oldenv params args (addVariable vpname val state)
buildArgs oldenv (param@(vpname,(vpdir,tname)):params) (arg@(Left vaname):args) state =
  case vpdir of
    Upd -> let val = getValueAlternate vaname dataSize oldenv state 
      in buildArgs oldenv params args (addVariable vpname val state)
    Out -> let val = getValueAlternate vaname dataSize oldenv state 
      in buildArgs oldenv params args (addVariable vpname val state)
